...
#'
#' @export
pivot_longer_values <- S7::new_generic(
  "pivot_longer_values",
  "data",
  function(
    data,
...
S7::method(
  ...
  names_to <- rlang::as_string(rlang::ensym(names_to))
  values_to <- rlang::as_string(rlang::ensym(values_to))

  survey_data <- data$variables
  cols <- get_col_names(survey_data, {{ cols }})
  ...
  rebuild_srvyr_design(long_df, data)
}
...
S7::method(
  ...
  rebuild_srvyr_rep_design(long_df, data)
}
